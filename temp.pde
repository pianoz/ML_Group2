...
/*
int[][] suckmyglobal=new int[8][10];

int smooooooth(int wave, int[] data){
  //first put all the data in the array and then refresh 
  for(int i=0; i<10; i++){
    //iterate through to add
    int temp = suckmyglobal[wave][i];
    if(i==0){
       suckmyglobal[wave][i]=data;
    }
    else{
      suckmyglobal]
    }
  }
  //next calculate the standard deviation b/c average will be 0 for a waveform usually
  //finally return row i of the array
  */
  return 1;
}
